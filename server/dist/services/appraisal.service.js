    ...
    static async getPacketDetail(packetId, userId, organizationId, userRank = 0) {
        ...
        try {
            ...
            const packet = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('The system connection timed out (12.5s). Please try again.')), 12500))
            ]);
                        ...
                        if (r.responses) {
                            try {
                                ...
                                if (data.competencyScores) {
                                    data.competencyScores = data.competencyScores.map((cat) => ({
                                        ...cat,
