...
async function setup() {
    ...
    if (md && hr)
        await prisma.user.update({ where: { id: hr.id }, data: { supervisorId: md.id } });
    if (md && it)
        await prisma.user.update({ where: { id: it.id }, data: { supervisorId: md.id } });
    if (md && director)
        await prisma.user.update({ where: { id: director.id }, data: { supervisorId: md.id } });
    if (director && manager)
        await prisma.user.update({ where: { id: manager.id }, data: { supervisorId: director.id } });
    if (manager && midMgr)
        await prisma.user.update({ where: { id: midMgr.id }, data: { supervisorId: manager.id } });
    if (midMgr && staff)
        await prisma.user.update({ where: { id: staff.id }, data: { supervisorId: midMgr.id } });
    if (midMgr && casual)
        await prisma.user.update({ where: { id: casual.id }, data: { supervisorId: midMgr.id } });
